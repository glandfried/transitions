...
\en{As a consequence of the non-ergodicity of multiplicative processes, fluctuations have a negative effect on individual growth rates, which can be reduced through mutual cooperation. }%
\es{Como consecuencia de la no-ergodicidad de los procesos multiplicativos, las fluctuaciones tienen un efecto negativo en las tasas de crecimiento individuales, las cuales pueden ser reducidas a través de la mutua cooperación. }%
%
...
\en{Yaari-Solomon and Peters-Adamou~\cite{yaari2010-cooperationEvolution, peters-cooperation2019.03.04} (hereinafter Yaari and Peters) considers the following cooperative strategy and analyzes the consequences it has on the growth rate of the agents. }%
\es{Yaari-Solomon and Peters-Adamou~\cite{peters-cooperation2019.03.04} (de aquí en adelante Yaari y Peters) consideran la siguiente estrategia cooperativa y analiza la consecuencias que tiene sobre la tasa de crecimiento de los agentes. }%
%
\begin{figure}[H]
\centering
\scalebox{0.75}{
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{
    \en{The resources of an individual from a group of 33 agents who share their wealth after each iteration (green line), approaches the expected value (black line). As a visual reference, we show the growth rate of the individuals (blue line). }%
    \es{Los recursos de un individuo de una grupo de 33 agentes que comparten su riqueza luego de cada iteración (recta verde), se pega al valor esperado (recta negra).
    Como referencia visual, dejamos la tasa de crecimiento de los individuos (recta azul). }%
    }
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{
    \en{Figure \ref{fig:multilevel-selection-7}: proportional growth rate in mixed groups of size 100. }%
    \es{Figura \ref{fig:multilevel-selection-7}: tasa de crecimiento proporcional en grupos mixtos de tamaño 100. }%
    %
...
\en{A major transition occurs when units that were previously capable of independent reproduction evolve into entities that can only replicate collectively. }%
\es{Una transición mayor ocurre cuando unidades que previemante eran capaces de replicarse de forma independiente evolucionan hacia entidades que sólo puede replicarse colectivamente. }%
%
\en{In general this involves division of labor. }%
\es{En general esto involucra división del trabajo. }%
%
\en{The reproductive division of labor is observed in almost all multicellular organisms and eusocial animals. }%
\es{La división reproductiva del trabajo se observa en casi todos los organismos multicelulares y los animales eusociales. }%

% Parrafo

\en{To study the division of labor we analyze what happens when individuals in the same group can acquire different strategies. }%
...
%
\en{That is, it is not convenient to change the strategy if we restrict the analysis to individuals who work with the same intensity, since the sum of their bets is $1$ regardless of the individual, $\Ee_i + (1_\Ee_i) = 1$. }%
\es{Es decir, no conveniene un cambio de estrategia si restringimos el análisis a individuos que trabajan con la misma intensidad, en tanto que la suma de sus apuestas es $1$ indistintamente del individuo $\Ee_i + (1_\Ee_i) = 1$. }%
%
\en{To analyze the division of labor in general terms we will allow individuals to transfer part of their tasks to others, while maintaining the same total restriction, $ \frac{1}{N} \sum_i^N \Ee_i + (1_\Ee_i) = 1$. }%
\es{Para analizar la división de trabajo en términos generales vamos a permitir que los individuos cedan parte de sus tareas a otros, manteniendo la misma restricción total, $ \frac{1}{N} \sum_i^N \Ee_i + (1_\Ee_i) = 1$. }% 
%
...
%
\en{Unlike cooperation and specialization, the multiplicative nature of probabilistic and evolutionary theories does not create an evolutionary advantage in favor of division of labor, it allows its evolutionary drift. }%
...
%
\en{This coincidence is an additional support to the hypothesis of isomorphism between evolutionary and probabilistic theories, previously identified between their fundamental equations: the Bayes theorem and the replicator dynamic~\cite{harper2009-replicatorAsInference,shalizi2009-replicatorAsInference}. }%
