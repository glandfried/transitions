...
%opening
\title{\vspace{-2.2cm}Multilevel selection in causal models: the multiplicative nature of evolutionary and probabilistic selection processes as the general driver for the irreversible emergence of cooperation and specialization.\footnote{\url{https://github.com/glandfried/transitions/releases/download/doc1.0.1/landfried-\en{transitions}\es{transiciones}.pdf}} }
...
% %
\en{In this section we will extend the causal model proposed in section ``\nameref{sec:methodology}'' by incorporating unconditionally cooperative behaviors, such as the one presented in the ``\nameref{sec:introduction}'' section, including defecting behaviors (which receive the benefit of cooperation without contributing to it). }%
...
%
\en{As we have seen in the section ``\nameref{sec:methodology}'', the probability of the environmental states is, }%
\es{Como hemos visto en la sección ``\nameref{sec:methodology}'', la probabilidad de los estados ambientales es, }%
%
\begin{equation}
P(\Aa) = p^\Aa (1-p)^{1-\Aa}
\end{equation}
...
%
\en{Then, the posteriors of individuals are}\es{Luego} (\en{a detailed mathematical proof can be found in the appendix section ``\nameref{sec:probabilistic_inference}''}\es{una demostración matemática detallada se encuentran en la sección ``\nameref{sec:probabilistic_inference}'' del apéndice.})
%
\begin{equation}
P(k| \vec{\Aa}^{\,1}, \dots, \vec{\Aa}^T) = 
\begin{cases}
%P(k)\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) &  r=0  \\
P(k)\prod_{t=1}^{T} \sum_j^{\texttt{\en{partners}\es{socios}}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,t}) & \texttt{coop}(k)  \\ % r = N
\Big(P(k)\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) \Big) + \Big(\sum_{t=1}^{T} P(c|\wedge_{q=1}^t\vec{\Aa}^{\,q})  \prod_{q=t+1}^T P(k|\vec{\Aa}^{\,q}) \Big) & \neg\texttt{coop}(k) % 0 < r < N  
\end{cases}
\end{equation}
%
...
%
\en{A detailed mathematical proof can be found in the appendix section ``\nameref{sec:peters_isomorphism}''. }%
\es{Una demostración matemática detallada se encuentran en la sección ``\nameref{sec:peters_isomorphism}'' del apéndice. }%
%
...
\end{equation}
%
\en{The proportionality between the posterior probability of individuals and the trajectories of resources studied in section ``\nameref{sec:introduction}'' allows us to work with both expressions interchangeably. }%
\es{La proporcionalidad entre la probabilidad a posteriori de los individuos y las trayectorias de los recursos estudiada en la ``\nameref{sec:introduction}'' nos permite trabajar indistintamanete con una u otra expresión. }%
...
%
\en{A detailed mathematical proof can be found in the appendix section ``\nameref{sec:growth_rates}''. }%
\es{Una demostración matemática detallada se encuentran en la sección `` \nameref{sec:growth_rates}'' del apéndice. }%
...
\subsection{\en{Posteriors of strategies in the basic causal model}\es{Posterior de las estrategias en el modelo causal básico}}
...
\subsection{\en{Posteriors of individuals in the extended causal model}\es{Posteriores de los individuos en el modelo causal extendido}}\label{sec:probabilistic_inference}

\en{The probability of individuals, when the environmental states $\vec{\Aa}^{\,t}$ are observed, is }%
\es{La probabilidad de los individuos cuando tenemos como observable todos los ambientes $\vec{\Aa}^{\,t}$ es, }%
%
\begin{equation}
...
\end{equation}
%
\en{where}\es{donde} (\en{all steps are described in the next section}
\es{todos los pasos estan descritos en la siguiente sección})
%
\begin{equation}
P(k| \vec{\Aa}^{\,1}, \dots, \vec{\Aa}^T) = 
\begin{cases}
...
%\en{Posteriors of the extended causal model}\es{Posteriores del modelo causal extendido}
\subsection{\en{Marginal distributions of the extenden causal model step by step}\es{Distribuciones marginales del modelo causal extendido paso a paso}}
...
%
\en{The marginal probabilities can be obtained by applying the sum-product algorithm in the model of figure~\ref{fig:multilevel_model}, taking as observed the variables that in the posterior appear in the conditional (see section ``\nameref{sec:methodology}''). }%
\es{Las probabilidades marginales se pueden obtener aplicando el sum-product algorithm en el modelo de la figura~\ref{fig:multilevel_model} pero que tiene como observables las variables que en el posterior aparecen en el condicional (ver sección ``\nameref{sec:methodology}''). }%
%
...
\subsection{\en{Isomorphism of the extended causal model}\es{Isomorfismo del modelo causal extendido}}\label{sec:peters_isomorphism}

\en{Our probabilistic causal model and the multiplicative process discussed in the introduction are equivalent. }%
\es{Nuestro modelo causal probabilístico y el proceso multiplicativo analizado en la introducción son equivalentes. }%
%
...
%
\en{When the prior $P(i^{T+1})$ represents the initial resources $\omega(0)$, then the posterior of the individual defectors in entirely defecting groups is proportional to the growth of the individual resources studied in section ``\nameref{sec:introduction}''. }%
\es{Si consideramos que el prior $P(i^{T+1})$ representa los recursos inciales $\omega(0)$, luego el posterior de los individuos desertores en grupos enteramenete desertoras es proporcional al crecimiento de los recursos individuales estudiada en la sección ``\nameref{sec:introduction}''. }%
...
\subsection{\en{Growth Rates}\es{Tasas de crecimiento}}\label{sec:growth_rates}
