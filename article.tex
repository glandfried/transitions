...
\end{equation}
%
una secuencias estocástica de tasas de supervivencia y reproducción $f(\cdot)$ dependientes de un ambiente aleatorio $r$.
...
Es posible mostrar que formulaciones aparentemente diferentes entre sí son variantes de un modelo de crecimiento exponencial conocido como \emph{replicator dynamic} \cite{taylor1978-replicatorDynamic, schuster1983-replicatorDynamics,hofbauer2003-evolutionaryGameDynamics}.
...
Sea $\overline{f}_i(x)$ la estimación de la tasa de crecimiento $r_i$, la proporción de la estrategia $i$ en la población en el siguiente paso es,    
...
Buena parte de la literatura en evolución considera que la estimación correcta de la tasa de crecimiento es el valor esperado las posibles trayectorias individuales de los recursos en el tiempo, $\omega(t)$.
...
Los dos primeros pasos temporales es 
\begin{equation}
\begin{split}
...
El valor esperado en cualquier tiempo $t$ se obtiene a través de una tasa de crecimiento de $5\%$ por cada paso temporal, $\langle \omega \rangle_t = 1.05^t$.
Y efectivamente, en una muestra suficientemente grande de trayectoria individuales de los recursos en el tiempo, ese es el promedio observado.
...
Sin embargo, en el largo plazo todas las trayectorias individuales de los recursos pierden a una tasa cercana al 5\%.
...
El valor esperado no representa lo que lo le ocurre a los agentes en el tiempo.
Esta diferencia es un problema bien conocido en mecánica estadística, llamado ``no-ergodicidad''~\cite{peters2019-ergodicityEconomics}.
...
Ante la duda, siempre conviene analizar directamente la dinámica. 
En nuestro caso, los recursos están gobernados por un procesos multiplicativos, que en un cierto tiempo $t$ puede expresarse como,
\begin{equation}
...
En el límite, $T \rightarrow \infty$ todas las trayectoria individuales estarán gobernadas por el mismo factor efectivo $\overline{f}$ por el que se multiplica los recursos $\omega(t)$ en cada iteración.
\begin{equation}
\begin{split}
...
Esta fórmula, que permite computar el promedio temporal de las trayectorias individuales en los procesos multiplicativos, se la conoce también como \emph{media geométrica} y ha sido usada 
en la literatura de evolución~\cite{dempster1955-geometricMean}.
El hecho de que el promedio temporal sea menor a promedio de estados tiene que ver con que en los procesos multiplicativos los impactos físicos de las pérdidas suelen ser más fuertes que los de las ganancias.
