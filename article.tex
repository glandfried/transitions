...
En nuestro caso la media geométrica es $\sqrt{1.5 \cdot 0.6} \approx 0.95$, lo que coincide con las tasas de crecimiento observadas en la figura \ref{fig:cpr_individual}
...
Según Ole Peters, la naturaleza no-ergódica de los procesos evolutivos sería la principal explicación de las transiciones evolutivas observadas en la historia de la vida.
Si bien Ole Peters muestra que al compartir recursos, las estrategias cooperadoras reducen sus fluctuaciones, aumentando sus tasas de crecimiento a largo plazo, no analiza el problema de los free-riders.
Esto no es resuelto por Ole Peters, quien dice, ``Our cooperators are unable to break the cooperative pact''.

Para discutir este punto en detalle, analizamos las tasas de crecimiento de las estrategias cooperadora y desertora cuando se encuentran de a pares.
Las tasas de crecimiento temporal estimada son:
...
Un agente que ``decide'' desertar unilateralmente va a ver reducida su tasa de crecimiento de $r(C,C) = 1.0$ a $ r(D,C) = 0.95$, lo cual apoyar la idea de Ole Peters de que la cooperación no es altruista sino que está impulsada por el interés personal.
Sin embargo, los agentes evolutivos no deciden como los agentes económicos, su frecuencia cambia en base a tasa de diferencial de crecimiento.
Por más que la mutua cooperación sea mejor en términos absolutos que la deserción unilateral ($r(C,C) = 1.0 > 0.95 = r(D,C)$), un mutante desertor invadirá evolutivamente la población debido a que su tasa de crecimiento es mayor a la de la tasa del cooperador ($r(D,C) = 0.95 > 0.47 = r(C,D)$).
Esto quiere decir que las estrategias cooperadoras no son evolutivamentes estables al interior de las poblaciones  \footnote{La estructura de pagos coinicide con la matriz de pagos del Stag-Hunt. Los análisis sobre la estabilidad en este tipo de juego llegan en general a la conclusión de que tanto las poblaciones enteramente cooperadoras como las enteramente desertoras son estrategias evolutivamente estables. Esto conclusión que difiere con nuestro resultado es consecuencia de los supuesto utilizados, dinámicas aditivas y poblaciones infinitas, los cuales son rechazados en este trabajo por supuestos más simples basados en procesos multiplicativos ruidosos y poblaciones finitas.}.
Ole Peters no provée entonces una respuesta que explique por qué las estrategias cooperadoras son favorecidas en términos evolutivos.
...
Un grupo de 2 cooperadores, un grupo de 1 cooperador y 1 desertor, un grupos de desertores.
...
P(e_{t}|e_{t-1}) = \begin{tabular}{|c|c|c|c|c|c|c|}
       ...
       $e_{t-1}=2$ & $0.5$ & $0.5$ & $0$ & $0$ & $0$ & $0$ \\ \hline
       $e_{t-1}=3$ & $0$ & $0$ & $0.5$ & $0.5$ & $0$ & $0$ \\ \hline
       $e_{t-1}=4$ & $0$ & $0$ & $0$ & $1.0$ & $0$ & $0$ \\ \hline
       $e_{t-1}=5$ & $0$ & $0$ & $0$ & $0$ & $1.0$ & $0$ \\ \hline
