...
%
Los individuos están caracterizados por tres atributos: el vector de estrategias $\vec{\Ee} = (\Ee_1, \dots, \Ee_M)$, que es un parámetro del modelo (en el caso más simple, todos los individuos tienen la misma estrategia); la región a la que pertenece, $\texttt{region}(i) = i \ \texttt{div} \ N$, definido como la división entera por $N$; y el comportamiento social, $\texttt{coop}(i) = i \ \texttt{mod} \ N < \texttt{region}(i)$, los primeros $n$ individuos del grupo son cooperadores y el resto desertores.
%
Como antes, la variable ambiente $\A$ sale cara con probabilidad $p$ y sello con probabilidad $1-p$, $P(\Aa) = p^\Aa (1-p)^{1-\Aa}$, pero ahora en cada tiempo $t$ tenemos un vector $\vec{\Aa}$, en el que cada elemento influencia al $i-$ésimo agente, $P(i|\vec{\Aa}) = \Ee_i^{\Aa_i} (1-\Ee_i)^{1-\Aa_i}$.
%
...
%
Para calcularlo lo resolveremos por casos debido a que el factor social es diferente según la región sea enteramente desertora, enteramente cooperadora o mixta.
%
...
%
En el anexo lo resolvemos por casos mediante inducción matemática: primero para los individuos de la región enteramente desertora, luego para la región enteramente cooperadores, y finalmente para la región mixta.
%
...
\begin{cases}
\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) &  r=0  \\
\prod_{t=1}^{T} \sum_j^{\texttt{socios}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,t}) & r  = N  \\
\Big(\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) \Big) + \Big(\sum_{t=1}^{T} P(c|\wedge_{q=1}^t\vec{\Aa}^{\,q})  \prod_{q=t+1}^T P(k|\vec{\Aa}^{\,q}) \Big) & 0 < r < N  
\end{cases}
\end{equation}
%
...
%
\todo{Agregar regiones al modelo para que el prior de los grupos sea 0.5 cuando hay 3 regiones, y que solo se necesiten N individuos, y no M}
\begin{figure}[H]
    \centering
...
%
\begin{equation}
 m_{P(I^{T+1}|I^{T}) \rightarrow I^{T+1} }(k) \overset{\text{HI}_d}{=} \prod_{t=1}^{T} P(\vec{\Aa}^{\,t}) \, P(i^{T+1}|\vec{\Aa}^{\,t})
\end{equation}
%
...
\begin{split}
P(i^{T+1} | \vec{\Aa}^{\,1}, \dots, \vec{\Aa}^{\,T}) \overset{\hfrac{\text{caso}}{1}}{=} \prod_{t=1}^{T} P(i^{T+1}|\vec{\Aa}^{\,t}) & \propto \prod_{t=1}^{T} f_{\Ee_i}(\Aa_{i}^{t})
\end{split}
\end{equation}
%
con $i = i^{T+1}$.
