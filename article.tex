...
\paragraph{Regiones enteramente desertoras}
...
\paragraph{Regiones enteramente cooperadoras}
...
\paragraph{Regiones mixtas, individuos cooperadores}
...
\paragraph{Regiones mixtas, individuos desertores}

Antes de proponer una hipótesis inductiva, veamos qué con los primero mensajes pasos de modo de ganar intuición.
%
Por definición, 
%
\begin{equation}
...
\begin{equation}
\begin{split}
m_{P(I^{2}|I^{1}) \rightarrow I^{2} }(k) = P(\vec{\Aa}^{\,1}) \Big( \underbrace{P(k|\vec{\Aa}^{\,1})\phantom{\Bigg|_j} }_{s_D(1)} + \underbrace{\sum_j^{\texttt{socios}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,1})}_{s_C(1)} \Big) 
\end{split}
\end{equation}
%
Donde $s_D(t)$ y $s_C(t)$ son las variables aleatorias que reciben los individuos en regiones enteramente desertoras y cooperadoras en el tiempo $t$.  
%
Por definición, el siguiente mensaje 
%
\begin{equation}
\begin{split}
m_{P(I^{3}|I^{2}) \rightarrow I^{3} }(k) &= \sum_{i^2} P(k|i^2)  P(\vec{\Aa}^{\,2}) P(i^2|\vec{\Aa}^{\,2}) m_{P(I^{2}|I^{1}) \rightarrow I^{2} }(k) \\[-0.25cm]
&= \sum_{i^2} P(i^3|i^2) P(\vec{\Aa}^{\,2})P(i^2|\vec{\Aa}^{\,2}) \Bigg(P(\vec{\Aa}^{\,1}) \Big( P(k|\vec{\Aa}^{\,1}) + \sum_j^{\texttt{socios}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,1}) \Big) \Bigg) \\
&= P(\vec{\Aa}^{\,2})P(\vec{\Aa}^{\,1}) (s_D(1) + s_C(1)) \sum_{i^2} P(i^3|i^2) P(i^2|\vec{\Aa}^{\,2}) \\[-0.25cm]
&= P(\vec{\Aa}^{\,2})P(\vec{\Aa}^{\,1}) (s_D(1) + s_C(1)) \Big(P(k|\vec{\Aa}^{\,2}) + \sum_j^{\texttt{socios}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,2}) \Big) \\
&= P(\vec{\Aa}^{\,2})P(\vec{\Aa}^{\,1}) (s_D(1) + s_C(1)) (s_D(2) + s_C(2))  \\
\end{split}
\end{equation}
%
Luego, la hipótesis inductiva $\text{HI}_M(T)$ para individuos desertores $k$ de regiones $r$ mixtas es,
\begin{equation}
m_{P(I^{T+1}|I^{T}) \rightarrow I^{T+1} }(k) = \Bigg(\prod_{t=1}^{T} P(\vec{\Aa}^{\,2}) \Bigg) \Bigg(\prod_{t=1}^{T} s_D(t) + s_C(t) \Bigg) 
\end{equation}
%
...
que primero juega y después recibe la parte del fondo común.
\begin{equation}
P(\bm{a}^{:t},e|n,N, c_0) =
\begin{cases}
 P(a_0(1)|e) & \ \  t=1 \\
