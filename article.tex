...
%
\en{}%
\es{Otra forma de definir distribuciones conjuntas es mediante el uso de modelos gráficos, en los que hay un nodo por cada variable, y una flecha por cada par ``padre-hijo''. }% 
%
\en{Graphical models are a well-established tool in probability because they: 1. provide an intuitive language to unambiguously express all hypotheses; 2. are a low-dimensional representation of the joint probability distribution; 3. and allow efficient inference through the sum-product algorithm~\cite{kschischang2001-factorGraphsAndTheSumProductAlgorithm}.}
\es{Los modelos gráficos son una herramienta bien establecida en probabilidad debido a que permiten: 1. ofrecen un lenguaje intuitivo para expresar sin ambiguedad todas las hipótesis; 2. es una representación de baja dimensionalidad de la distribución de probabilidad conjunta; 3. y permite realizar inferencia de forma eficiente a través del algoritmos suma-producto~\cite{ kschischang2001-factorGraphsAndTheSumProductAlgorithm}. }%

% Parrafo

Para utilizar los modelos gráficos no se requiere una interpretación causal, de hecho las probabilidades condicionales pueden expresar en cualquier orden.
%
Sin embargo, son varias las ventajas de definir las probabilidades condicionales en base a interpretaciones causales~\cite{pearl2009-causality}.
%
La ciencia elabora sus teorías en base a historias causales, mecanismos físicos estables y autónomos, los cuales inducen probabilidades condicionales entre las causas y sus efectos.
%
En este sentido, justificar las probabilidades condicionales en base a una historia causal es una forma más natural de expresar lo que sabemos o creemos sobre el mundo.
%
Además de facilitar la intuición en el proceso de investigación, la interpretación causal permite modularizar las probabilidades condicionales.
%
Eventuales cambios en alguno de los mecanismos causales afecta solo localmente la topología de la red bayesiana, permitiendo predecir el efecto de las intervenciones externas con un mínimo de información adicional.
%
Por estos motivos, en este trabajo expresaremos los modelos gráficos en términos de relaciones causales. 
...
\begin{equation}\label{eq:bayes_theorem}
 \underbrace{p(\overbrace{\text{\en{Hypothesis}\es{Hipótesis}$_i$}}^{\text{\en{Hidden}\es{Oculta}}}|\overbrace{\text{Dat\en{a}\es{os}}}^{\text{Observ\en{ed}\es{ado}}}, \text{Model\es{o}})}_{\text{Posterior}} = \frac{\overbrace{P(\,\text{Dat\en{a}\es{os}}\,|\,\text{\en{Hypothesis}\es{Hipótesis}$_i$}\,,\text{Model\es{o}})}^{\text{\en{Likelihood}\es{Verosimilitud}}}\overbrace{p(\text{\en{Hypothesis}\es{Hipótesis}$_i$})}^{\text{Prior}}}{\underbrace{P(\text{Dat\en{a}\es{os}}\,|\,\text{Model\es{o}})}_{\text{Evidenc\en{e}\es{ia} o\en{r}\es{ predicci\'on a} prior \en{prediction}}}}
\end{equation}
%
\en{A posterior belief is nothing more than the prior belief that continues to be compatible with the data. }
\es{La creencia a posteriori no es más que la creencia a priori que continúa siendo compatible con los datos. }


Existe un isomorfismo entre las ecuaciones fundamentales de la teoría de la evolución (replicator dynamic) y la teoría de la probabilidad (teorema de bayes)~\cite{shalizi2009-replicatorAsInference,harper2009-replicatorAsInference},
%
\begin{align*}
\centering
 \begin{tabular}{l|l}
  Teorema de Bayes & Replicator dynamic  \\ \hline
  Prior $p(H)$ & Proporción previa $x_e$ \\ \hline
  Verosimilitud $p(D|H)$ & Fitness $f_e(a)$ \\ \hline
  Evidencia $p(D)$ & Población total $\sum x_e f_e(a) $ \\ \hline
  Posterior $p(H|D)$ & Proproción posterior $x_e^\prime$ \\ \hline
 \end{tabular}
\end{align*}
%
En base a este isomorfismo, Czégel, Zachar y Szathmáry (este último co-autor del concepto de trasiciones evolutivas mayores) propusieron recientemente analizar la evolución de una población sujeta a selección multinivel a través de modelo bayesianos jerárquicos.
%that (i) evolution of a hierarchical population under multilevel selection is equivalent to Bayesian inference in hierarchical Bayesian models and (ii) evolutionary transitions in individuality, driven by synergistic fitness interactions, is equivalent to learning the structure of hierarchical models via Bayesian model comparison
%
\en{However, beyond the proposal, the authors of this article (Czegel et al) fail to offer a model that represents multilevel selection, their examples are only pictorial. }%
\es{Sin embargo, más allá de la propuesta, los autores de este artículo (Czegel et al) no logran ofrecer un modelo que represente selección multinivel, sus ejemplos son sólo pictóricos. }%
%
