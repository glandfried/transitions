...
En este ejemplo la distribución de estrategias $x$ no afecta el valor de la aptitud, por lo que $f(r) = f(r|x)$.
Así definida, la la tasa de crecimiento es una variable aleatoria.
¿Cuál es entonces la tasa de crecimiento estimada (determinista) que se utiliza generalmente en el replicator dynamic?
...
Según el promedio de estados, el crecimiento esperado es de 5\%, $\langle f(x) \rangle = 1.05 = \sum_r f(r|x) p(r)$. 
Sin embargo, lo que efectivamente le ocurre a los agentes en el tiempo es muy diferente: a largo plazo todos pierden a una tasa de cercana de 5\%.
...
Esto ocurre porque en los proceso multiplicativos propios de los sistemas evolutivos (definidos mediante el replicator dynamic)  los impactos físicos de las pérdidas suelen ser más fuertes que los de las ganancias.
Esto se debe a que los sistemas son irreversibles.
En el extremo, un único cero en la secuencia tasas de reproducción y superviviencia, significa la extinción.

La diferencia entre lo que le ocurre a los agentes en el tiempo respecto de la esperanza de los estados del sistema es un problema bien conocido en la mecánica estadística, llamado ``no-ergodicidad''.
Sea $\omega(t) = \prod_i^t f(r_i)$ los recursos del agente en el tiempo $t$ y sea $\omega = f(r)$ los estados del sistema, decimos que un proceso es ergódico si se cumple que,
% \begin{equation}
%  \underbrace{\lim_{T \mapsto \infty} \frac{1}{T}\int_0^T f(\omega(t)) dt}_{\text{Media temporal}}  = \underbrace{\int_{\Omega} f(\omega)p(\omega) d\omega}_{\text{Media de estados}}
% \end{equation}
\begin{equation}
 \underbrace{\lim_{T \mapsto \infty} \frac{1}{T} \sum_t^T \omega(t)}_{\text{Media temporal}}  = \underbrace{\sum_{\omega} \omega \cdot p(\omega)}_{\text{Media de estados}}
\end{equation}

donde $r$ representa los estados del sistema, $f(r)$ la variable aleatoria, $r(t)$ el estado del sistema obtenido aleatoriamente en el tiempo $t$ y $p(r)$ la probabilidad del estado $r$.
