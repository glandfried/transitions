...
%
Los individuos están caracterizados por tres atributos: el vector de estrategias $\bm{\Ee} = (\Ee_1, \dots, \Ee_M)$, que es un parámetro del modelo (en el caso más simple, todos los individuos tienen la misma estrategia); por el grupo al que pertenecen, $P(g|i) = \mathbb{I}(i \texttt{ div } N = g)$, si la división entera por $N$ es igual al valor del grupo entonces pertenece a ese grupo con probabilidad $1$, sino con probabilidad $0$; y por el comportamiento social, los primeros $n$ individuos del grupo son cooperadores y el resto desertores.
%
Como antes, el ambiente $\A$ es una variable independiente que con probabilidad $p$ sale cara y con probabilidad $1-p$ sale sello, $P(\Aa) = p^\Aa (1-p)^{1-\Aa}$, pero ahora necesitamos un vector $\bm{\Aa}$, los agentes están influenciados por el $i-$ésimo elemento del vector ambiente, $P(i|\bm{\Aa}) = \bm{\Ee}_i^{\bm{\Aa}_i} (1-\bm{\Ee}_i)^{\bm{\Aa}_i$.
%
...
P(i^{t+1}|i^t) = 
\begin{cases}
1/N & \texttt{esCooperador}(i^t) \text{ y } \texttt{mismoGrupo}(i^{t+1},i^{t}) \\
0 & \texttt{esCooperador}(i^t) \text{ y } \neg \texttt{mismoGrupo}(i^{t+1},i^{t}) \\
1 & \neg \texttt{esCooperador}(i^t)
\end{cases}
\end{equation}
%
...
\centering
 ...
 \tikz{
    ...
    \node[latent] (a1) {$\vec{\A}^{\,1}$};
    \node[latent, right=of a1] (a2) {$\vec{\A}^{\,2}$};
...
\begin{subfigure}[c]{0.48\textwidth}    
 ...
 \tikz{
    ...
    \node[factor] (fa1) {};
    \node[latent, yshift=0.5cm, below=of fa1] (a1) {$\vec{\A}^{\,1}$};
    ...
    \node[latent, yshift=-0.5cm, above=of fia2] (a2) {$\vec{\A}^{\,1}$};
    ...
    \node[latent, yshift=0.5cm, below=of fg3 ] (g3) {$G^2$};
...
%
Para empezar, analicemos lo que ocurre en grupos enteramente desertores y cooperadores.
La probabilidad marginal de los individuos y el ambiente en grupos desertores, 
%
\begin{equation}\label{eq:des_temporal_average}
\begin{split}
P(i, \bm{\Aa}^1, \dots, \bm{\Aa}^T| g_0) &= \prod_t^T P(\bm{\Aa}) P(i|\bm{\Aa}^t_i) \\
& \propto  \prod_t^T P(i|\bm{\Aa}^t_i) \approx  P(i|\bm{\Aa}^t_i=1)^{p} P(i|\bm{\Aa}^t_i=0)^{1-p}
\end{split}
\end{equation}
%
...
En una grupos enteramente cooperadores la aptitud de los agentes depende del tamaño $N$ del grupo.
Par ejemplificar, supongamos que tenemos grupos de tamaño $N=3$.
En cada paso temporal, la probabilidad marginal de los cooperadores es,
\begin{equation}
\begin{split}
