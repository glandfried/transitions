...
%opening
\title{Multilevel selection in causal cooperative processes: nonergodicity of evolutionary and inferential systems as the general driver of cooperation and specialization.}
%Selección multinivel en procesos cooperativos causales: la no-ergodicidad de los sistemas evolutivos e inferenciales como el impulsor general de la cooperación y la especialización
...
%
\en{The reduction in resources occurs even for the second individual who changes from cooperative to defector behavior: the resources of the defector individuals in the group with 2 defectors are below the resources of the cooperative individuals in the group with 1 defector. }%
...
%
\en{Mutual defection produces a posterior that is updated according to the following product, }%
\es{La mutua deserción produce un posterior que se actualiza en función de la siguiente productoria, }%
%
\begin{equation}
\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) & \propto \prod_{t=1}^{T} \Ee_k^{\Aa_k} (1-\Ee_k)^{1-\Aa_k} 
\end{equation}
%
\en{The growth rate of a strategy $\Ee= 0.71 \approx 1.5/2.1$ in an environment with $p=0.5$ is $g_{DD} = 0.71^{1/2}\cdot0.29^{1/2} \approx 0.452$. }%
\es{La tasa de crecimiento de un estrategia $\Ee= 0.71 \approx 1.5/2.1$ en una ambiente con $p=0.5$ es $g_{DD} = 0.71^{1/2}\cdot0.29^{1/2} \approx 0.452$. }%
%
\en{The posterior of mutual cooperation is also updated with a product, but the random variable at each step is the average of the results of the partners, }%
\es{El posterior de la mutua cooperación también se actualiza en función de una productoria, pero la variable aleatoria en cada paso es el promedio de los resultados los socios, }%
%
\begin{equation}
\prod_{t=1}^{T} \sum_j^{\texttt{\en{partners}\es{socios}}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,t}) \propto \prod_{t=1}^{T} \underbrace{ \sum_j^{\texttt{\en{partners}\es{socios}}(r)} \frac{1}{N} \Ee_k^{\Aa_k} (1-\Ee_k)^{1-\Aa_k} }_{f_{C}(\Ee,\vec{\Aa}\,,N)} 
\end{equation}
%
\en{where the proportional is valid because of the normalization constant of the probability distribution $P(i^t|\vec{\Aa}^{\,t})$. }%
\es{donde el proporcional vale por constante de normalización de la distribución de probabilidad $ P(i^t|\vec{\Aa}^{\,t})$. }%
%
\en{When all members of the group have the same strategy $\Ee$, the possible values of the cooperative random variable is, }%
\es{Cuando todos los miembros del grupo tienen la misma estrategia $\Ee$, los posibles valores de la variable aleatoria cooperativa es, }%
% %
% \begin{equation}
% f_C(\Ee,\vec{\Aa}\,) =
% \begin{cases}
...
\end{equation}
%
\en{The cooperative growth rate of a strategy $\Ee=0.71$ in a group of size 2 in an environment with probability $p=0.5$, is $0.71^{1/4}\cdot 0.5^{1/2} \cdot 0.29^{1/4} \approx 0.475$. }%
\es{La tasa de crecimiento cooperativa de una estrategia $\Ee=0.71$ en un grupo de tamaño 2 en un ambiente con probabilidad $p=0.5$, $0.71^{1/4}\cdot 0.5^{1/2} \cdot 0.29^{1/4} \approx 0.475$. }%
%
\en{When the population is very large, $N\rightarrow \infty$, in each round there will invariably be a $p$ proproportion that succeeded, so the growth rate is reduced to the arithmetic mean, }%
\es{Cuando la población es muy grande, $N\rightarrow \infty$, en cada ronda habrá invariantemente una proproción $p$ que obtuvo éxito, por lo que la tasa de crecimiento se reduce a la media aritmética, }%
%
\begin{equation}
\lim_{N\rightarrow \infty} g_{C}(\Ee,N) = p \Ee + (1-p)(1-\Ee)
\end{equation}
%
...
%
\en{The growth rate of cooperative individuals in mixed populations is easily extended, }%
\es{La tasa de crecimiento de los individuos cooperadores en poblaciones mixtas se extiende facilmente, }%
%
\begin{equation}
g_{CD}(\Ee,n,N) = \prod_{x=0}^n \Big(\frac{x}{N} \Ee + \frac{N-x}{N}(1-\Ee)\Big)^{\text{Binomial}(x|n,p)}
\end{equation}
%
\en{The cooperative growth rate of a strategy $\Ee=0.71$ in a mixed group of size 2 in an environment with probability $p=0.5$ is, $g_{CD}(0.71,1,2)=(\frac{0.71}{2})^{1/4}\cdot (\frac{0.29}{2})^{1/2}  \approx 0.226$. }%
\es{La tasa de crecimiento cooperativa de una estrategia $\Ee=0.71$ en un grupo mixto de tamaño 2 en un ambiente con probabilidad $p=0.5$ es $g_{CD}(0.71,1,2)=(\frac{0.71}{2})^{1/4}\cdot (\frac{0.29}{2})^{1/2}  \approx 0.226$. }%
%
\en{The posterior of defector individuals in mixed populations is updated as a function of, }%
\es{El posterior de los individuos desertores en poblaciones mixtas se actualiza en función de, }%
%
\begin{equation}
\begin{split}
& \Big(\prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) \Big) + \Big(\sum_{t=1}^{T} \prod_{q=1}^{t} \sum_j^{\texttt{\en{partners}\es{socios}}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,q})  \prod_{q=t+1}^T P(k|\vec{\Aa}^{\,q}) \Big) \\
\end{split}
\end{equation}
%
\en{the sum of individual growth plus a moving average of the growth of cooperators weighted by individual growth. }%
\es{la suma del crecimiento individual más una media movil del crecimiento de los cooperadores pesado por crecimienti individual. }%
%
\en{Each of the elements updating the posterior of defecting individuals in mixed regions can be estimated using the growth rates of defecting individuals in entirely defecting regions $g_{D}$ and cooperating individuals in mixed regions $g_{CD}(\Ee,n,N)$. }%
\es{Cada uno de los elementos que actualizan el posterior de los individuos desertores en regiones mixtas se pueden estimar utilizar las tasas de crecimiento de los desertores en regiones enteramente desertoras $g_{D}$ e individuos cooperadores en regiones mixtas $g_{CD}(\Ee,n,N)$. }%
%
...
\end{equation}
%
\en{This growth rate stabilizes after a dozen time steps at the larger growth rate, $g_{D}(\Ee,n,N) \approx \texttt{max}(g_{D}(\Ee), g_{C}(\Ee,n,N))$. }%
\es{Esta tasa de crecimiento se estabiliza luego de una decena de pasos temporales en la tasa de crecimiento que sea mayor, $g_{D}(\Ee,n,N) \approx \texttt{max}(g_{D}(\Ee), g_{C}(\Ee,n,N))$. }%
%
\en{In figure \ref{fig:multilevel-selection-7} we show the growth rate of defectors as a function of the proportion of total defectors in a group of size $1000$. }%
\es{En la figura \ref{fig:multilevel-selection-7} mostramos la tasa de crecimiento de los desertores en función de la proproción de desertores totales en una grupo de tamaño $1000$. }%
%
\en{The growth rate of defecting individuals in mixed regions, $g_{D}(\Ee,n,N)$, remains the same as the growth rate of cooperating individuals, $g_{C}(\Ee,n,N)$, until the proportion of defectors reduces the growth rate of cooperators below the growth rate of entirely defecting regions, $g_{D}(\Ee)$. }%
\es{La tasa de crecimiento de los individuos desertores en regiones mixtas, $g_{D}(\Ee,n,N)$, se mantiene igual que la tasa de crecimiento de los individuos cooperdora, $g_{C}(\Ee,n,N)$, hasta que la proporcipon de desertores reduce la tasa crecimiento de los cooperadores por debajo de la tasa de crecimiento de los regiones enteramente desertoras, $g_{D}(\Ee)$. }%
%
\en{In Figure~\ref{fig:multilevel-selection-5} we see that the estimated growth rate, rescaled by the $2.1$ factor, overlaps over the resource trajectories presented in Figure~\ref{fig:ergodicity_desertion}. }
\es{En la figura~\ref{fig:multilevel-selection-5} vemos que la tasa de crecimiento estimada, reescladas por el factor $2.1$, se superpone sobre las trayectorias de los recursos presentado en la figura \ref{fig:ergodicity_desertion}. }%
%
\begin{figure}[H]
    \centering
    ...
    \end{subfigure}
    \caption{
    \en{Figure \ref{fig:multilevel-selection-7}: estimation of growth rate in mixed groups of size 100. }%
    \es{Figura \ref{fig:multilevel-selection-7}: estimación de la tasa de crecimiento en grupos mixtos de tamaño 100. }%
    %
    \en{Figure \ref{fig:multilevel-selection-5}: trajectory of the resources of a defector (blue) and cooperator (green) in a group of size 100 with a single defector (see figure~\ref{fig:ergodicity_desertion}), the black curves are the estimates of the growth rates, and the dotted black curve are the resources of cooperators in regions without defectors. }%
...
\en{Without the need to introduce penalties, defector strategies negatively affect their own long-term growth rate because their own behavior increases the fluctuations of the random variable on which they depend. }%
\es{Sin necesidad de introducir castigos, las estrategias desertoras afectan negativamente su propia tasa de crecimiento a largo plazo debido que su propio comportamiento aumento las fluctuaciones de la variable aleatoria de la que dependen.}
\end{conclution}
%
...
%
\en{For this reason, multilevel selection favors cooperators over defectors, even though the presence of defectors negatively affects their growth rate. }%
\es{Por este motivo, la selección multinivel favorece a los individuos cooperadores sobre los desertores, a pesar de que la presencia de destores afecte negativa su tasa de crecimiento. }%
