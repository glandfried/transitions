...
\end{equation}
%
Para definir la probabilidad de las estrategia dado el ambiente, $P(\Ee|\Aa)$, necesitamos que para cada ambiente la probabilidad de todas las estrategias sume 1.
%
...
\tikz{
    ...
    \node[const, right=of r] (rn) {$P(\Ee|\Aa) \propto f_\Ee(\Aa)$};
...
\tikz{
    ...
    \node[const, above=of fe] (nfe) {$P(\Ee|\Aa) \propto f_{\Ee}(\Aa)$};
...
\subsection{Dilema social}

En las últimas décadas la biología evolutiva a comenzado a adoptar la analogía de la ``tragedia de los comunes'' en relación al estudio de la evolución de la cooperación y de las trasiciones evolutivas mayores~\cite{rankin2007-tragedyCommonsBiology}.
%
El concepto de ``tragedia de los comunes'' contiene la creencia de que los bienes comunes tienen una estructura de pagos isomorfa al dilema del prisionero de N jugadores~\cite{hardin1971-collectiveAsPrisionerDilema}.
%
Imaginemos dos jugadores, cada uno de los cuales tiene que decidir si coopera o no.
%
Cooperar (C) implica un coste $c$, para que la otra persona reciba un beneficio b, con $b > c$.
%
Desistir (D) significa negarse a cooperar y no conlleva ningún coste. 
%
De este modo, se obtiene la siguiente matriz de pagos:
%
\begin{equation}
  \bordermatrix{ & C & D \cr
      C & b-c & -c \cr
...
Para verificar la creencia de que la dinámica de bienes comunes produce matrices de pagos isomórfas al dilema del prisionero, analicemos la media temporal de los posteriors en el tiempo.
%
La mutua deserción produce un posterior que es proporcional al proceso multipicativo estudiado por Ole Peters,
%
\begin{equation}
P(k | \vec{\Aa}^{\,1}, \dots, \vec{\Aa}^{\,T}) \overset{\hfrac{\text{caso}}{1}}{=} P(k) \prod_{t=1}^{T} P(k|\vec{\Aa}^{\,t}) \\
& \propto P(k) \prod_{t=1}^{T} f_{\Ee_i}(\Aa_{i}^{t})
\end{equation}
%
donde el proporcional vale por construcción de la distribución de probabilidad $ P(i^t|\vec{\Aa}^{\,t})$ (ver ecuación~\ref{eq:probabilidad_propro_aptitud}).
%
En la introducción vimos que en una ambiente con $p=0.5$ y un individuo con estrategia $\Ee=1.5$ y recursos totales $R=2.1$ es $\approx 0.95$.

% Parrafo

De modo similar, el posterior de mutua cooperación es proporcional al proceso cooperativo estudiado por Ole Peters.
%
\begin{equation}
P(i^{T+1}|\vec{\Aa}^{\,1},\dots,\vec{\Aa}^{\,T}) \overset{\hfrac{}{\text{caso}}}{=} \prod_{t=1}^{T} \underbrace{\sum_j^{\texttt{socios}(r)} \frac{1}{N} P(j|\vec{\Aa}^{\,t})}_{s_C(\vec{\Aa}\,)} \propto \prod_{t=1}^{T} \sum_j^{\texttt{socios}(r)} \frac{1}{N} f_{\Ee_j}(\Aa_{j}^{t}) = \omega_C(T)
\end{equation}
%
donde el proporcional vale por construcción de la distribución de probabilidad $ P(i^t|\vec{\Aa}^{\,t})$.
%
Notar la diferencia entre el posterior de mutua cooperación y el de mutua deserción.
%
